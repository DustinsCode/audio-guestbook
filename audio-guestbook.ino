...
void setup() {
  ...
  Serial.println("Testing, about to beep?");
  ...
  MTP.addFilesystem(SD, "Ian and Kaydee's Audio guestbook");  // choose a nice name for the SD card volume to appear in your file explorer
...
void loop() {
  ...
  switch (mode) {
    case Mode::Ready:
      ...
      if (buttonRecord.fallingEdge() && buttonPlay.read() == 1) {
        Serial.println("Handset lifted");
        mode = Mode::Prompting;
        print_mode();
      } else if (buttonRecord.fallingEdge() && buttonPlay.read() == 0) {
        playAllRecordings();
        // playLastRecording();
      }
      break;
